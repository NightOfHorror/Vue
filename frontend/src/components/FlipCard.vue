<template>
    <div class="flip-card" @click="handleCardClick" :class="{ 'dark-mode': $store.getters.isDarkMode }">
      <div class="flip-card-inner" :class="{ flipped: isFlipped }">
        ...
        <div class="flip-card-back" :class="{ correctAnswer: isFlipped && isCorrectAnswer, wrongAnswer: isFlipped && !isCorrectAnswer }">
          ...
          <div style="font-size:11px;"> {{ currentQuestion.explication }}</div>
        </div>
      </div>
    </div>
  </template>
  ...
  .flip-card {
    ...
    cursor: pointer;
    margin: auto; /* Ajout de la propriété pour centrer la carte */
    display: flex;
  ...
  /* Ajout de la classe dark-mode pour le mode sombre */
  .dark-mode .flip-card {
